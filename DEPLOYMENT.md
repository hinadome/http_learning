...
- **Rate limiting** — `/api/send`, `/api/ws`, and `/api/mqtt` enforce per-IP fixed windows (in-memory, per instance): 30/min Send, 15/min WS and MQTT by default. Override with `RATE_LIMIT_SEND_MAX`, `RATE_LIMIT_WS_MAX`, `RATE_LIMIT_MQTT_MAX`, `RATE_LIMIT_WINDOW_MS`. Clients receive **429** + `Retry-After`.
